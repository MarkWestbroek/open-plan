...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       git clone git@bitbucket.org:maykinmedia/openplan.git
       cd openplan
...
``src/openplan/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    python src/manage.py test openplan
...
* ``DB_NAME``: name of the database for the project. Defaults to ``openplan``.
* ``DB_USER``: username to connect to the database with. Defaults to ``openplan``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``openplan``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       docker-compose up -d
       Starting openplan_db_1 ... done
       Starting openplan_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       docker exec -it openplan_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       docker exec -it openplan_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/openplan/
...
environment variables. See ``src/openplan/conf/docker.py`` for
all settings.
...
.. code-block:: bash
    ...
    docker exec -it openplan /app/src/manage.py createsuperuser
