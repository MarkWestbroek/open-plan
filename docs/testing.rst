...
1. ``openplan-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``openplan-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``openplan-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``openplan-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``openplan-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``openplan-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``openplan-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``openplan-pr-isort``: checks that imports are correctly
