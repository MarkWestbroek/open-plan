...
beheerinterface van Open Plan (hierna *de admin* genoemd).

In deze handleiding nemen we aan dat Open Plan geïnstalleerd is en beschikbaar
op het adres https://open-plan.gemeente.nl.
...
https://open-plan.gemeente.nl. Klik vervolgens op **Beheer**:
...
De *API's Open Plan* zijn niet toegankelijk zonder autorisatie.
Dit betekent dat elke applicatie die gegevens ophaalt of registreert in Open
Plan hiervoor geautoriseerd moet zijn. Dit kan via een token.
...
Het configuratiegedeelte dient om de Open Plan-installatie te configureren.
...
anders worden er door Open Plan geen notificaties verstuurd.
...
Er worden vaak Plan logberichten weggeschreven die kunnen wijzen op een
probleem in de Open Plan applicatie. Deze worden via de logs inzichtelijk
gemaakt.

Misc
----

**Websites** bevat gegevens over waar Open Plan gehost wordt. Zorg ervoor dat
...
2. Klik een onderwerp aan binnen een groep, bijvoorbeeld *Plannen*
...
Als voorbeeld zie je de lijstweergave van *Plannen*:

.. image:: assets/plannen_list.png
    :width: 100%
    :alt: Plannenlijst
...
Als voorbeeld zie je (een deel van) de detailweergave van een plan:

.. image:: assets/plannen_detail.png
    :width: 100%
    :alt: Plan detail

1. De attributen van het plan worden opgelijst als bewerkbare velden. Sommige
