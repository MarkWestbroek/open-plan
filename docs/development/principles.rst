...
- All apps must go in the ``src/openplan`` directory, which namespaces all the Open Plan
  code in the ``openplan`` package. This prevents name conflicts with third party
  applications.

- Application names should always be in plural form.

- Settings go in ``openplan.conf``, which is split according to deploy environment:
  ...
  Settings must always be defined in the ``openplan.conf.base`` with sane defaults.

- Global runtime Open Plan configuration (database backed) should go in the
  ``openplan.config`` app.

- Generic tools that are used by specific apps should be a ``openplan`` sub-package,
  or possibly go in ``openplan.utils``.

- Integration with other, third-party services/interfaces should go in a
  ``openplan.contrib`` package. This is currently (!) not the case yet.
